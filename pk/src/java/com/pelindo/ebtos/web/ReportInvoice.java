 ...
 * @author dycoder
 */
@WebServlet(name="Invoice", urlPatterns={"/invoice.pdf"})
public class ReportInvoice extends HttpServlet {
    @EJB IDGeneratorFacadeRemote iDGeneratorFacade;
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            if(curr.equalsIgnoreCase("ID")){
                BigDecimal total = new BigDecimal(to);
                terbilang = converter.convertToWord(total.toBigInteger().toString()) + "Rupiah";
            } else {
            ...
            if (tipe.equalsIgnoreCase("SC002")) {
            ...
            } else if (tipe.equalsIgnoreCase("SC022")) {
                report = loader.getResourceAsStream("/com/pelindo/ebtos/report/InvoiceUnderPayment.jasper");
                if(detail.equalsIgnoreCase("true"))
                ...
                report = loader.getResourceAsStream("/com/pelindo/ebtos/report/InvoicePenumpukanSusulan.jasper");
                if(detail.equalsIgnoreCase("true"))
                ...
                report = loader.getResourceAsStream("/com/pelindo/ebtos/report/ChangeStatus.jasper");
                response.setHeader("Content-Disposition", "inline; filename=\"InvoiceChangeStatus.pdf\"");
            } else {
    ...
    private static void close(Closeable resource) {
        if (resource != null) {
            try {
                ...
                Logger.getLogger(ReportInvoice.class.getName()).log(Level.SEVERE, null, e);
            }
        }
    }
