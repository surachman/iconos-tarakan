...
/*    
    @NamedNativeQuery(name = "RecapShifting.Native.findHandlingInvoice", query = "SELECT a.no_ppkb, ((SELECT nvl(SUM(s.total_charge)) FROM recap_shifting s WHERE a.no_ppkb=s.no_ppkb AND s.operation ='DISCHARGE')+"
                ...
                + "GROUP BY a.no_ppkb"),
*/
    @NamedNativeQuery(name = "RecapShifting.Native.findHandlingInvoice", query = 
"SELECT a.no_ppkb, " 
+"  ( " 
+"  (SELECT nvln(SUM(s.total_charge)) " 
+"  FROM recap_shifting s " 
+"  WHERE a.no_ppkb =s.no_ppkb " 
+"  AND s.operation ='DISCHARGE' " 
+"  ) + " 
+"  (SELECT nvln(SUM(m.total_charge)) " 
+"  FROM recap_hatch_move m " 
+"  WHERE a.no_ppkb = m.no_ppkb " 
+"  AND m.operation ='DISCHARGE' " 
+"  ) + " 
+"  (SELECT nvln(SUM(t.total_charge)) " 
+"  FROM recap_transhipment t " 
+"  WHERE a.no_ppkb = t.no_ppkb " 
+"  AND t.operation ='DISCHARGE' " 
+"  )) AS paket_handling, " 
+"  (SELECT nvln(SUM(p.total_charge)) " 
