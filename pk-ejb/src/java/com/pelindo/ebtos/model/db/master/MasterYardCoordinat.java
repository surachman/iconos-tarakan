...
@NamedQueries({
    ...
    @NamedQuery(name = "MasterYardCoordinat.updateMasterYardContainerGateOut", query = "UPDATE MasterYardCoordinat m SET m.contNo = NULL, m.contType = NULL, m.contSize = NULL, m.noPpkb = NULL, m.contWeight = NULL, m.status = 'empty', m.grossClass = NULL, m.blNo = NULL, m.mlo = NULL, m.pod = NULL WHERE m.contNo = :contNo AND m.noPpkb = :noPpkb"),
...
@NamedNativeQueries({
    ...
    + "JOIN m_pattern mp ON (mctig.pattern=mp.name)) ON (myc.cont_type=mct.cont_type) "
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockSlotAndPpkb",query= 
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
+"LEFT JOIN m_gross_class mgc " 
+"ON (mgc.gross_class=myc.gross_class)) " 
+"LEFT JOIN (m_container_type mct " 
+"JOIN m_container_type_in_general mctig " 
+"ON (mct.type_in_general=mctig.id) " 
+"JOIN m_pattern mp " 
+"ON (mctig.pattern           =mp.name)) " 
+"ON (to_number(myc.cont_type)=mct.cont_type) " 
+"WHERE block                 =? " 
+"AND status                 <>'empty' " 
+"AND slot                    =? " 
+"AND no_ppkb LIKE ('%' " 
...
/*                                                                                            
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockTierAndPpkb",query="SELECT " +
                                                                                    ...
                                                                                    "ORDER BY " +
                                                                                            "myc.cont_size, myc.cont_no, myc.slot"),
*/                                                                                            
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockTierAndPpkb",query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
*/                                                                                            
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockAndSlotExistOnly",query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockSlotAndPpkbExistOnly",query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
+"    myc.pod, " 
+"    CASE " 
+"      WHEN obj.slot    <> myc.slot " 
+"      AND myc.cont_size = 40 " 
+"      THEN 1 " 
+"      ELSE 0 " 
+"    END is_tail " 
+"  FROM " 
...
+"  ON (to_number(myc.cont_type)=mct.cont_type)) " 
+"  WHERE myc.block             =? " 
+"  AND myc.status              = 'exist' " 
+"  AND (myc.slot               =obj.slot " 
+"  OR (myc.slot                =obj.slot-1 " 
+"  AND myc.cont_size           = 40)) " 
+"  AND myc.no_ppkb LIKE ('%' " 
+"    || ? " 
+"    || '%') " 
+"  ) myc " 
+"LEFT JOIN " 
...
+"    v_tier " 
+"  ) pcl " 
+"ON (myc.no_ppkb=pcl.no_ppkb " 
+"AND myc.cont_no=pcl.cont_no) " 
+"WHERE myc.slot =pcl.y_slot"),
...
/*
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockAndTierExistOnly",query="SELECT " +
                                                                                    ...
                                                                                    "ORDER BY " +
                                                                                            "myc.cont_size, myc.cont_no, myc.slot"),
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContByBlockAndTierExistOnly",query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
/*
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContBfindByContNoStatusExistyBlockTierAndPpkbExistOnly",query="SELECT " +
                                                                                                    ...
                                                                                                    "ORDER BY " +
                                                                                                            "myc.cont_size, myc.cont_no, myc.slot"),
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findContBfindByContNoStatusExistyBlockTierAndPpkbExistOnly",query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
+"  ) pcl " 
+"ON (myc.no_ppkb =pcl.no_ppkb " 
+"AND myc.block   =pcl.block " 
+"AND myc.slot    =pcl.y_slot " 
+"AND myc.\"ROW\" =pcl.y_row " 
+"AND myc.tier    =pcl.y_tier) " 
+"WHERE myc.block =? " 
+"AND myc.status  = 'exist' " 
+"AND myc.tier    =? " 
+"AND myc.no_ppkb LIKE ('%' " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findOverallContByBlock",query=
"SELECT myc.slot, " 
+"  myc.\"ROW\", " 
+"  SUM( " 
+"  CASE " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.isBottomAvailable",query=
"SELECT (CASE WHEN p.tier = 1 " 
+"OR (COUNT( " 
+"  CASE " 
+"    WHEN myc.cont_no IS NOT NULL " 
+"    THEN 1 " 
+"  END) = p.teus) THEN 'TRUE' ELSE 'FALSE' END) is_bottom_available " 
+"FROM " 
+"  ( SELECT TO_CHAR(?) block, ? slot, ? \"ROW\", ? tier, ? teus FROM DUAL " 
+"  ) p " 
+"LEFT JOIN m_yard_coordinat myc " 
+"ON (myc.block   = p.block " 
+"AND myc.status  = 'exist' " 
+"AND myc.\"ROW\" = p.\"ROW\" " 
+"AND myc.tier    = (p.tier               - 1) " 
+"AND myc.slot BETWEEN p.slot AND (p.slot + p.teus - 1)) " 
+"GROUP BY p.tier, " 
+"  p.teus"),
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.isTopEmpty",query=
"SELECT ( " 
+"  CASE " 
+"    WHEN p.tier = my.tier " 
+"    OR (COUNT( " 
+"      CASE " 
+"        WHEN myc.status IS NOT NULL " 
+"        THEN 1 " 
+"      END) = p.teus) " 
+"    THEN 'TRUE' " 
+"    ELSE 'FALSE' " 
...
+"LEFT JOIN m_yard_coordinat myc " 
+"ON (myc.block   = p.block " 
+"AND myc.status IN ('planning', 'empty') " 
+"AND myc.\"ROW\" = p.\"ROW\" " 
+"AND myc.tier    = (p.tier               + 1) " 
+"AND myc.slot BETWEEN p.slot AND (p.slot + p.teus - 1)) " 
+"JOIN m_yard my " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findOverallContByBlockColors",query=
"SELECT myc.slot, " 
+"  myc.\"ROW\", " 
+"  SUM( " 
+"  CASE " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findIdStatusByCoordinat",query=    
"SELECT id, " 
+"  status, " 
+"  cont_no, " 
+"  no_ppkb " 
+"FROM m_yard_coordinat " 
+"WHERE block =? " 
+"AND slot    =? " 
+"AND \"ROW\" =? " 
+"AND tier    =?"),
...
*/    
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findGenerate", query=
"SELECT a.cont_no, " 
+"  a.cont_size, " 
+"  b.type_in_general AS name, " 
+"  a.cont_weight, " 
+"  c.description, " 
+"  a.block, " 
+"  a.slot, " 
...
+"  a.tier, " 
+"  a.id " 
+"FROM m_yard_coordinat a, " 
+"  m_container_type b, " 
+"  m_gross_class c "   
+"WHERE to_number(a.cont_type) = b.cont_type " 
+"AND a.gross_class            =c.gross_class " 
+"AND a.id                    IN " 
+"  (SELECT id_coordinat " 
+"  FROM yard_allocation_temp " 
+"  WHERE no_ppkb       = ? " 
+"  AND is_transhipment = ? " 
+"  AND is_shifting     = ? " 
+"  ) " 
+"ORDER BY a.id"),
    ...
    @NamedNativeQuery(name="MasterYardCoordinat.Native.unFinishBayPlanDischargeByPPKBCont", query="SELECT DISTINCT cont_no FROM m_yard_coordinat WHERE id IN (SELECT DISTINCT id_coordinat FROM yard_allocation_temp WHERE no_ppkb=? AND id_coordinat IS NOT NULL)"),
...
*/    
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findDuplicate", query=
"SELECT cont_no, " 
+"  block, " 
+"  slot, " 
+"  \"ROW\", " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findByContNoStatusExist", query=
"SELECT myc.cont_no, " 
+"  myc.cont_type, " 
+"  myc.cont_size, " 
+"  myc.cont_weight, " 
...
*/
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findPlannedCoordinateByNoPpkbAndDestination", query=
"SELECT myc.id " 
+"FROM m_yard_coordinat myc " 
+"JOIN " 
+"  (SELECT pya.no_ppkb, " 
...
*/
    ...
    @NamedNativeQuery(name="MasterYardCoordinat.Native.findAvailableCoordinateIdByAllocationConstraint", query=
"SELECT myc.id " 
+"FROM m_yard_coordinat myc " 
+"JOIN " 
+"  (SELECT pya.no_ppkb, " 
