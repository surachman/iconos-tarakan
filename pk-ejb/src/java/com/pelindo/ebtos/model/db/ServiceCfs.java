...
*/
    @NamedNativeQuery(name = "ServiceCfs.Native.findStripping", query = 
"SELECT sc.id, " 
+"  sc.no_ppkb, " 
+"  sc.cont_no, " 
+"  sc.commodity_code, " 
...
+"ON (sc.cont_no        = scd.cont_no " 
+"AND sc.no_ppkb        = scd.no_ppkb) " 
+"WHERE sc.end_time    IS NULL " 
+"AND scd.is_cfs = 'TRUE'"),    

    
    @NamedNativeQuery(name = "ServiceCfs.Native.findStuffing", query = "SELECT sc.id, sc.no_ppkb, sc.cont_no, sc.commodity_code FROM service_cfs sc  where sc.OPERATION = 'STUFFING' and sc.end_time    IS NULL"), 
...
*/  
    @NamedNativeQuery(name = "ServiceCfs.Native.findIsLast", query = 
"SELECT id " 
+"FROM service_cfs " 
+"WHERE no_ppkb = ? " 
+"AND cont_no       = ? " 
+"AND nvln(is_last) = 'TRUE'")    
})
