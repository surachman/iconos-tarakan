...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findVesselChangeAbleContainers", query = 
"SELECT sr.cont_no, " 
+"  sr.cont_size, " 
+"  mct.type_in_general, " 
+"  sr.cont_status, " 
...
+"  sr.y_tier, " 
+"  sr.cont_gross, " 
+"  (SELECT g.gross_class " 
+"  FROM m_gross_parameter g " 
+"  WHERE g.cont_size  =sr.cont_size " 
+"  AND sr.cont_gross >= g.min_gross " 
+"  AND sr.cont_gross  < g.max_gross " 
+"  ) AS gross_class, " 
+"  mc.name commodity_name " 
+"FROM service_receiving sr " 
+"LEFT JOIN service_cont_load scl " 
+"ON (sr.cont_no=scl.cont_no " 
+"AND sr.no_ppkb=scl.no_ppkb) " 
+"JOIN m_container_type mct " 
+"ON (sr.cont_type=mct.cont_type) " 
+"JOIN planning_cont_load pcl " 
+"ON (sr.cont_no=pcl.cont_no " 
+"AND sr.no_ppkb=pcl.no_ppkb) " 
+"JOIN receiving_service rs " 
+"ON (sr.cont_no=rs.cont_no " 
+"AND sr.no_ppkb=rs.no_ppkb) " 
+"INNER JOIN m_commodity mc " 
+"ON sr.commodity_code           = mc.commodity_code " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingByPpkbList2", query = 
"SELECT r.cont_no, " 
+"  r.cont_size, " 
...
+"  r.block, " 
+"  r.is_lifton " 
+"FROM service_receiving r, " 
+"  m_container_type m " 
+"WHERE r.cont_type          =m.cont_type " 
+"AND r.no_ppkb              = ? " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingByPpkbList", query = 
"SELECT r.id, " 
+"  r.commodity_code, " 
+"  r.block, " 
+"  r.cont_type, " 
...
+"FROM service_receiving r " 
+"WHERE r.no_ppkb    = ? " 
+"AND r.cont_no NOT IN " 
+"  (SELECT cont_no FROM planning_cont_load WHERE no_ppkb=r.no_ppkb " 
+"  ) " 
+"AND r.status_cancel_loading= 'FALSE' " 
+"ORDER BY r.id DESC"),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingByPpkbReefer", query = 
"SELECT sr.cont_no, " 
+"  sr.cont_size, " 
+"  mct.name, " 
+"  sr.cont_status, " 
...
+"JOIN m_container_type mct " 
+"ON (sr.cont_type = mct.cont_type) " 
+"LEFT JOIN service_reefer srf " 
+"ON (srf.no_ppkb           = sr.no_ppkb " 
+"AND srf.cont_no           = sr.cont_no) " 
+"WHERE mct.type_in_general = 'RFR' " 
+"AND sr.no_ppkb            = ? " 
+"AND srf.cont_no          IS NULL " 
+"ORDER BY sr.id DESC"),
...
/*
    @NamedNativeQuery(name = "ServiceReceiving.Native.findContainerReefer", query = "SELECT sr.cont_type, sr.cont_no, sr.cont_size, sr.cont_status, sr.no_ppkb "
                                                                                    ...
                                                                                    + "WHERE mct.type_in_general = 'RFR' AND sr.no_ppkb = ? AND srf.cont_no IS NULL ORDER BY sr.id DESC"),
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findContainerReefer", query = 
"SELECT sr.cont_type, " 
+"  sr.cont_no, " 
+"  sr.cont_size, " 
+"  sr.cont_status, " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingContNo", query = 
"SELECT scd.cont_no, " 
+"  scd.id " 
+"FROM service_receiving scd " 
+"WHERE scd.no_ppkb    = ? " 
+"AND scd.cont_no      = ? " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingByAutoComplete", query = 
"SELECT scd.cont_no " 
+"FROM service_receiving scd " 
+"WHERE scd.no_ppkb = ? " 
...
/*    
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsStuffingService", query = "SELECT p.cont_no, p.cont_size, c.name, p.cont_status, p.cont_gross, com.name, p.over_size, p.dangerous, p.dg_label, p.bl_no, p.mlo, p.id "
                                                                                                   ...
                                                                                                   + "ORDER BY p.id DESC"),
    */
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsStuffingService", query = 
"SELECT p.cont_no, " 
+"  p.cont_size, " 
+"  c.name, " 
+"  p.cont_status, " 
...
+"  p.mlo, " 
+"  p.id " 
+"FROM service_receiving p " 
+"JOIN m_container_type c " 
+"ON (p.cont_type=c.cont_type) " 
+"JOIN m_commodity com " 
+"ON (p.commodity_code = com.commodity_code) " 
+"LEFT JOIN stuffing_service ss " 
+"ON (ss.cont_no = p.cont_no) " 
...
+"AND p.no_ppkb                = ? " 
+"AND p.status_cancel_loading <> 'TRUE' " 
+"ORDER BY p.id DESC"),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsMovement", query = 
"SELECT p.cont_no, " 
+"  p.cont_size, " 
+"  c.name, " 
+"  p.cont_status, " 
...
+"  p.is_inspection, " 
+"  CASE " 
+"    WHEN is_behandle  = 'FALSE' " 
+"    AND is_cfs        = 'FALSE' " 
+"    AND is_inspection = 'FALSE' " 
+"    THEN 'TRUE' " 
+"    ELSE 'FALSE' " 
+"  END AS is_cy " 
+"FROM service_receiving p, " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsStuffingServiceByBL", query = 
"SELECT p.cont_no, " 
+"  p.cont_size, " 
+"  c.name, " 
+"  p.cont_status, " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsChangeStatus", query = 
"SELECT scd.id, " 
+"  scd.bl_no, " 
+"  scd.cont_no, " 
+"  mct.name, " 
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingsChangeDestination", query = 
"SELECT scd.id, " 
+"  scd.bl_no, " 
+"  scd.cont_no, " 
+"  mct.name, " 
...
+"WHERE scd.cont_type          = mct.cont_type " 
+"AND scd.commodity_code       =mc.commodity_code " 
+"AND scd.is_change_destination='TRUE' " 
+"AND scd.no_ppkb              =? " 
+"ORDER BY scd.id DESC"),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingByDestination", query = 
"SELECT r.id, " 
+"  r.old_ppkb " 
+"FROM service_receiving r " 
+"WHERE r.is_change_destination= 'TRUE' " 
+"AND r.old_ppkb               = ? " 
+"AND r.cont_no                = ?"),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingFrontByPPKBVersi2", query = 
"SELECT s.cont_no, " 
+"  s.cont_size, " 
+"  mct.type_in_general, " 
+"  s.cont_status, " 
...
+"ON mct.cont_type=s.cont_type " 
+"LEFT JOIN service_receiving sr " 
+"ON sr.cont_no                 =s.cont_no " 
+"AND sr.no_ppkb                =s.no_ppkb " 
+"AND sr.status_cancel_loading <> 'TRUE' " 
+"LEFT JOIN equipment e2 " 
+"ON s.cont_no              = e2.cont_no " 
+"AND s.no_ppkb             = e2.no_ppkb " 
+"AND e2.operation          = 'LOAD' " 
+"AND e2.equipment_act_code = 'LIFTON' " 
+"LEFT JOIN equipment e3 " 
+"ON s.cont_no              = e3.cont_no " 
+"AND s.no_ppkb             = e3.no_ppkb " 
+"AND e3.operation          = 'LOAD' " 
+"AND e3.equipment_act_code = 'H/T' " 
+"LEFT JOIN equipment e4 " 
+"ON s.cont_no              = e4.cont_no " 
+"AND s.no_ppkb             = e4.no_ppkb " 
+"AND e4.operation          = 'LOAD' " 
+"AND e4.equipment_act_code = 'STEVEDORING' " 
+"WHERE s.no_ppkb           = ? " 
+"AND s.position            ='01'"),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findServiceReceivingFrontByTrackingCont", query = 
"SELECT p.id, " 
+"  p.no_ppkb, " 
+"  p.bl_no, " 
+"  p.cont_no, " 
...
+"  p.y_tier, " 
+"  CASE " 
+"    WHEN p.cont_no = sl.cont_no " 
+"    AND p.no_ppkb  =sl.no_ppkb " 
+"    THEN 'pos' " 
+"      ||sl.position " 
+"    ELSE 'pos' " 
+"      ||p.position " 
...
+"LEFT JOIN service_cont_load sl " 
+"ON p.no_ppkb   = sl.no_ppkb " 
+"AND p.cont_no  =sl.cont_no) " 
+"WHERE p.no_ppkb= ? " 
+"AND p.cont_no  = ? " 
+"AND ROWNUM     < 2 " 
+"ORDER BY p.id DESC" ),
...
*/
    @NamedNativeQuery(name = "ServiceReceiving.Native.findYardLoad", query = 
...
+"ON e1.no_ppkb_recv        = sr.no_ppkb " 
+"AND e1.cont_no            = sr.cont_no " 
+"AND e1.equipment_act_code = 'RECEIVING' " 
...
public class ServiceReceiving implements Serializable, EntityAuditable {
    ...
    private String twentyOneFeet = "FALSE";//false;
    ...
    private String statusCancelLoading;
    @Basic(optional = false)
    @Column(name = "ex_discharge_to_load", nullable = false)
    private String exDischargeToLoad = "FALSE";//false;
    @OneToOne
    @JoinColumns(value = {
...
//    private boolean isSling = Boolean.FALSE;
    ...
    public ServiceReceiving(ServiceContDischarge serviceContDischarge) {
        ...
        this.exStuffing = "TRUE"; //Boolean.TRUE;
        this.isLifton = "FALSE"; //Boolean.FALSE;
    ...
    public String isExDischargeToLoad() {
        return exDischargeToLoad;
    }

    public void setExDischargeToLoad(String exDischargeToLoad) {
