...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findPlanningContReceivingsByNoPpkb", query = "SELECT r.cont_no, r.cont_size, mt.name, r.cont_status, r.cont_gross, r.seal_no,r.commodity_code,r.over_size,r.dg,r.id from planning_cont_receiving r,m_container_type mt,receiving_service rc WHERE r.no_ppkb = ? AND r.no_ppkb =rc.no_ppkb AND r.cont_no=rc.cont_no AND r.cont_type=mt.cont_type AND r.cont_no NOT IN (select cont_no from service_receiving where no_ppkb=r.no_ppkb) ORDER BY r.id ASC"),
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findReceivingConfirm", query = 
    "SELECT r.cont_no, " 
	+"  r.cont_size, " 
	+"  mt.name, " 
	+"  r.cont_status, " 
	...
	+"JOIN receiving_service rc " 
	+"ON (r.no_ppkb =rc.no_ppkb " 
	+"AND r.cont_no =rc.cont_no) " 
	+"LEFT JOIN " 
	+"  (SELECT cont_no, " 
	+"    no_ppkb " 
	+"  FROM service_receiving " 
	+"  WHERE status_cancel_loading <> 'TRUE' " 
	+"  ) sr " 
	+"ON (r.no_ppkb = sr.no_ppkb " 
	+"AND r.cont_no = sr.cont_no) " 
	+"JOIN service_gate_receiving sgr " 
	+"ON (rc.job_slip               =sgr.job_slip) " 
	+"WHERE sr.cont_no             IS NULL " 
	+"AND rc.status_cancel_loading <> 'TRUE' " 
	+"AND r.no_ppkb                 = ? " 
	+"ORDER BY r.id ASC"),
    //penambahan pencarian list no container by ade chelsea tanggal 24 maret 2014
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findReceivingConfirm2", query = 
    "SELECT r.cont_no, " 
	+"  r.cont_size, " 
	+"  mt.name, " 
	+"  r.cont_status, " 
...
/*	
    ...
    + "r.cont_type, r.cont_status, r.cont_gross, r.seal_no, r.block, r.fr_slot, r.fr_row FROM planning_cont_receiving r WHERE r.no_ppkb = ?"),
// MKR, Wed 12 Nov 2014 11:37:29 PM WIT 
// There are no such fields named as block, fr_slot, fr_row in planning_cont_receiving
// This Query will generate error!   
*/
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findPlanningContReceivingByNoPpkb", query = 
"SELECT r.cont_no, " 
+"  r.cont_size, " 
+"  r.cont_type, " 
+"  r.cont_status, " 
...
+"  planning_vessel s, " 
+"  receiving_service r, " 
+"  planning_receiving pr " 
+"WHERE p.no_ppkb    = s.no_ppkb " 
+"AND p.no_ppkb      = r.no_ppkb " 
+"AND p.cont_no      = r.cont_no " 
+"AND p.cont_no NOT IN " 
+"  (SELECT cont_no FROM service_receiving WHERE no_ppkb = p.no_ppkb " 
+"  ) " 
+"AND p.no_ppkb  = ? " 
+"AND p.cont_no  = ? " 
+"AND p.position = ? " 
+"AND (s.status  = 'Approved' " 
+"OR s.status    = 'Servicing' " 
+"OR s.status    = 'ReadyService') " 
+"AND p.cont_no IN " 
+"  (SELECT cont_no FROM service_gate_receiving WHERE no_ppkb=p.no_ppkb " 
+"  ) " 
+"AND pr.receiving_allocation_id = p.id_constrain_planning " 
+"AND rownum < 2"),
...
*/
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findPlanningContReceivingBaplieLoad", query = 
"SELECT r.id, " 
+"  m.name, " 
+"  mt.name, " 
+"  r.no_ppkb, " 
...
+"AND r.commodity_code=m.commodity_code " 
+"AND r.no_ppkb       = ? " 
+"AND r.is_completed IS NOT NULL " 
+"ORDER BY r.id DESC"),
	...
	+"ON (r.cont_no                  =rs.cont_no " 
	+"AND r.no_ppkb                  =rs.no_ppkb) " 
	+"WHERE r.cont_no                =? " 
	+"AND r.no_ppkb                  = ? " 
	+"AND (rs.status_cancel_loading IS NULL " 
	+"OR rs.status_cancel_loading    = 'FALSE')"),
    ...
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findByContNoMobile", query = "SELECT p.id, p.cont_no, p.no_ppkb FROM planning_cont_receiving p, planning_vessel s, receiving_service r WHERE p.no_ppkb = s.no_ppkb AND p.no_ppkb = r.no_ppkb AND p.cont_no = r.cont_no AND p.cont_no NOT IN (select cont_no from service_receiving where no_ppkb = p.no_ppkb) AND p.cont_no = ? AND p.position = ? AND s.close_rec > sysdate AND (s.status = 'Approved' OR s.status = 'Servicing' OR s.status = 'ReadyService')"),
    ...
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findContIds", query = "SELECT * " 
																															+"FROM xmltable( '/ROWSET/ROW' PASSING xmltype(FIND_CONTID(?,?,?)) columns ID PATH 'ID' )"),
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findPKs", query = "SELECT * " 
																															+"FROM xmltable( '/ROWSET/ROW' PASSING xmltype(FIND_PK(?,?,?,?)) columns FIND_PK PATH 'FIND_PK')"),
...
/*                                                                                            
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findReceivableContainerWithSuggestedLocation", query = "SELECT p.id, p.cont_no, p.no_ppkb, rng.block, rng.slot, rng.row, rng.tier, CASE WHEN rng.tier <= 3 THEN 0 ELSE -1 END AS ordering, p.cont_size, p.cont_status, p.over_size, p.dg, p.disch_port, sgr.cont_weight, mcd.name, mv.name, sgr.cont_no IS NOT NULL has_entering_gate "
                                                                                            ...
                                                                                            + "LIMIT 1"),
*/
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findReceivableContainerWithSuggestedLocation", query = 
"SELECT p.id, " 
+"  p.cont_no, " 
+"  p.no_ppkb, " 
+"  rng.block, " 
...
+"FROM planning_cont_receiving p " 
+"JOIN planning_vessel s " 
+"ON (p.no_ppkb = s.no_ppkb) " 
+"JOIN m_vessel mv " 
+"ON (s.vessel_code=mv.vessel_code) " 
+"JOIN receiving_service r " 
+"ON (p.no_ppkb = r.no_ppkb " 
+"AND p.cont_no = r.cont_no) " 
+"LEFT JOIN (service_gate_receiving sgr " 
+"JOIN m_cont_damage mcd " 
+"ON (sgr.cont_damage=mcd.id)) " 
+"ON (p.cont_no      =sgr.cont_no " 
+"AND p.no_ppkb      =sgr.no_ppkb) " 
+"LEFT JOIN service_receiving sr " 
+"ON (p.cont_no                 =sr.cont_no " 
+"AND p.no_ppkb                 =sr.no_ppkb " 
...
+"  ON (myc.no_ppkb=pya.no_ppkb " 
+"  AND myc.block  = pya.block " 
...
+"  ) rng ON (p.no_ppkb        =rng.no_ppkb " 
+"AND p.disch_port             =rng.port_code " 
+"AND p.gross_class            =rng.gross_class " 
+"AND p.cont_size              =rng.cont_size " 
+"AND p.cont_status            =rng.cont_status " 
+"AND p.over_size              =rng.over_size " 
+"AND p.dg                     =rng.dg) " 
+"JOIN m_container_type mc " 
+"ON (p.cont_type              =mc.cont_type) " 
+"WHERE sr.cont_no            IS NULL " 
+"AND sr.no_ppkb              IS NULL " 
+"AND p.cont_no                = ? " 
+"AND p.position               = '01' " 
+"AND (s.status                = 'Approved' " 
+"OR s.status                  = 'Servicing' " 
...
+"AND ROWNUM                   < 2 " 
+"ORDER BY rng.block ASC, " 
+"  ordering DESC, " 
+"  rng.slot ASC, " 
+"  rng.tier ASC, " 
+"  rng.\"ROW\" ASC"),
                                                                                            
    @NamedNativeQuery(name = "PlanningContReceiving.Native.findByContValidasiConstraint", query = "SELECT r.id FROM planning_cont_receiving r where r.no_ppkb= ? AND r.cont_size=? and r.cont_type=? and r.cont_status=? and r.over_size=? and r.dg=? and r.gross_class=? AND r.disch_port=? AND r.is_export=? AND rownum < 2")
})
