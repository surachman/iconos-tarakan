...
*/
    @NamedNativeQuery(name = "ReeferLoadService.Native.findByPpkbNReg", query = 
"SELECT rds.job_slip, " 
+"  c.name, " 
+"  rds.cont_no, " 
+"  rds.cont_size, " 
...
+"AND rds.no_reg           = ?"),    
    ...
    @NamedNativeQuery(name = "ReeferLoadService.Native.generateId", query = "SELECT MAX(substr(job_slip,7,5))FROM reefer_load_service WHERE substr(job_slip,3,4) = ?"),
...
*/
    ...
    @NamedNativeQuery(name = "ReeferLoadService.Native.perhitungan", query = 
"SELECT ds.job_slip, " 
+"  ds.cont_no, " 
+"  ds.cont_status, " 
+"  ds.cont_size, " 
+"  ds.cont_type, " 
+"  ds.plug_on_date, " 
+"  NVLN(ds.shift_planning), " 
+"  NVLN(pp.charge), " 
+"  NVLN(pp.total_charge), " 
+"  NVLN(pm.charge), " 
+"  NVLN(pm.total_charge), " 
+"  c.symbol, " 
+"  c.language, " 
+"  c.country, " 
...
*/    
    @NamedNativeQuery(name = "ReeferLoadService.Native.findByReg", query = 
"SELECT prs.job_slip, " 
+"  prs.cont_no, " 
+"  prs.cont_size, " 
...
+"  prs.cont_gross, " 
+"  CASE " 
+"    WHEN prs.over_size  = 'TRUE' " 
+"    THEN 'Yes' " 
+"    ELSE'No' " 
+"  END AS over_size " 
+"FROM reefer_load_service prs, " 
+"  m_container_type ct " 
+"WHERE prs.cont_type = ct.cont_type " 
+"AND prs.no_reg      = ?"),
...
*/    
    @NamedNativeQuery(name = "ReeferLoadService.Native.findGateReeferByJobSlip", query = 
"SELECT pp.job_slip, " 
+"  pp.no_ppkb, " 
+"  pp.cont_no, " 
+"  mm.iso_code, " 
...
*/
    @NamedNativeQuery(name = "ReeferLoadService.Native.findReeferLoadServiceByAutoComplete", query = 
"SELECT d.job_slip " 
+"FROM reefer_load_service d " 
