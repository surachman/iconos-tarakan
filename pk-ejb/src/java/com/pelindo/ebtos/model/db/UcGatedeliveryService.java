...
/*
    @NamedNativeQuery(name = "UcGatedeliveryService.Native.findUcGatedeliveryServiceByAutoComplete", query = "SELECT ugs.jobslip "
                                                                                                            ...
                                                                                                            + "LIMIT 10;"),
*/
    @NamedNativeQuery(name = "UcGatedeliveryService.Native.findUcGatedeliveryServiceByAutoComplete", query = 
"SELECT jobslip " 
+"FROM " 
+"  (SELECT ugs.jobslip " 
+"  FROM uc_gatedelivery_service ugs " 
+"  JOIN uc_delivery_service uds " 
+"  ON (ugs.jobslip=uds.jobslip) " 
+"  JOIN uncontainerized_service us " 
+"  ON (uds.id_uc=us.id_uc) " 
+"  WHERE ugs.jobslip LIKE ('%' " 
+"    || ? " 
+"    ||'%') " 
+"  AND ugs.date_out        IS NULL " 
+"  AND nvln(us.is_delivery) = 1 " 
+"  ORDER BY substring(ugs.jobslip,7,5) DESC " 
+"  ) " 
+"WHERE rownum <= 10"),                                                                                                            
/*
...
*/                                       
    ...
    @NamedNativeQuery(name = "UcGatedeliveryService.Native.findUcGatedeliveryServiceByDateOutNull", query = 
"SELECT ugs.jobslip, " 
+"  ugs.id_gate " 
+"FROM uc_gatedelivery_service ugs " 
+"JOIN uc_delivery_service uds " 
