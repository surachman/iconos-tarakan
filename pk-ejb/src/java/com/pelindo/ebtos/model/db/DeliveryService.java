...
@NamedQueries({
    ...
    @NamedQuery(name = "DeliveryService.findDataDelivery", query = "SELECT r FROM DeliveryService r where r.contNo = :contNo order by r.createdDate desc"),
...
*/    
    @NamedNativeQuery(name = "DeliveryService.Native.findByPpkbNReg", query = 
"SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
...
+"  ds.cont_gross, " 
+"  CASE " 
+"    WHEN NVL(ds.over_size,0)= 'TRUE' " 
+"    THEN 'Yes' " 
+"    WHEN NVL(ds.over_size,0)='FALSE' " 
+"    THEN 'No' " 
+"  END AS over_size, " 
+"  ds.placement_date " 
...
*/
    @NamedNativeQuery(name = "DeliveryService.Native.generateId", query = 
...
*/
    @NamedNativeQuery(name = "DeliveryService.Native.findDeliveryServiceByValidateDate", query = 
"SELECT pp.job_slip, " 
+"  pp.no_ppkb, " 
+"  pp.cont_no, " 
+"  mm.iso_code " 
...
+"ON (scd.cont_no=pp.cont_no " 
+"AND scd.no_ppkb=pp.no_ppkb) " 
+"JOIN m_commodity mc " 
+"ON (mc.commodity_code=scd.commodity_code) " 
+"LEFT JOIN service_gate_delivery sgd " 
+"ON (pp.job_slip            =sgd.job_slip) " 
+"WHERE pp.counter_print     > 0 " 
+"AND sgd.job_slip          IS NULL " 
+"AND ((scd.position         ='" + ServiceContDischarge.LOCATION_CY + "' " 
+"AND (mc.dangerous_class   IS NULL " 
+"OR mc.dangerous_class NOT IN ('1', '7'))) " 
+"OR (scd.position           ='" + ServiceContDischarge.LOCATION_VESSEL + "' " 
+"AND mc.dangerous_class    IN ('1', '7'))) " 
+"AND r.status_service       ='approve' " 
+"AND pp.cont_no             = ? "
+"AND pp.valid_date         >= sysdate"),
...
/*    
    @NamedNativeQuery(name = "DeliveryService.Native.perhitungan", query = "SELECT ds.job_slip, ds.cont_no, ds.cont_status, ds.cont_size, ct.type_in_general as name, change(ds.over_size) as over_size, change(ds.dg) as dg, change(ds.dg_label) as dg_label, ds.placement_date, nvl(pp.masa_1), nvl(pp.masa_2), pls.charge, nvl(pp.charge_m1), nvl(pp.charge_m2), nvl(phd.charge), nvl(ppg.total_charge), nvl(pp.charge), sgd.cont_no IS NOT NULL, nvl(pub.charge) "
                                                                            ...
                                                                            + "WHERE ds.no_reg = ?"),
*/
    @NamedNativeQuery(name = "DeliveryService.Native.perhitungan", query = 
"SELECT ds.job_slip,  \n" +
"   ds.cont_no,  \n" +
"   ds.cont_status,  \n" +
"   ds.cont_size,  \n" +
"   ct.type_in_general                   AS name,  \n" +
"   DECODE(ds.over_size, 'TRUE', 'Yes', 'No') AS over_size,  \n" +
"   DECODE(ds.dg, 'TRUE', 'Yes', 'No')        AS dg,  \n" +
"   DECODE(ds.dg_label, 'TRUE', 'Yes', 'No')  AS dg_label,  \n" +
"   ds.placement_date,  \n" +
"   NVL(ds.masa_1,0),  \n" +
"   NVL(ds.masa_2,0),  \n" +
"   NVL(ds.masa_3,0),  \n" +
...
*/
    @NamedNativeQuery(name = "DeliveryService.Native.findByPpkb", query =     
"SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
...
+"AND ds.cont_type        = ct.cont_type " 
+"AND ds.no_ppkb          = ? " 
+"AND ds.cont_no NOT     IN " 
+"  (SELECT cont_no FROM penumpukan_susulan_service WHERE no_ppkb = ds.no_ppkb " 
+"  )"),    
...
*/
    ...
    @NamedNativeQuery(name = "DeliveryService.Native.findByPpkbAndBlNo", query =     
"SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
...
*/    
    @NamedNativeQuery(name = "DeliveryService.Native.findByReg", query = 
"SELECT ds.job_slip, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
+"  ct.type_in_general AS name, " 
...
*/
    @NamedNativeQuery(name = "DeliveryService.Native.findByPpkbAndBlNoPenumpukanSusulan", query =     
"SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
...
+"AND scd.cont_no            =ds.cont_no " 
+"AND ds.valid_date         <= sysdate " 
+"AND NVL(scd.is_delivery,0) = 'FALSE'"),    
/*
...
*/
    ...
    @NamedNativeQuery(name = "DeliveryService.Native.findByPpkbPenumpukanSusulan", query = 
"SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
...
*/
    @NamedNativeQuery(name = "DeliveryService.Native.findCashierDischarge", query = 
"SELECT ds.job_slip, " 
+"  ds.valid_date, " 
+"  inv.no_invoice, " 
+"  inv.payment_date, " 
...
+"ON inv.no_reg = ds.no_reg " 
+"LEFT JOIN registration r " 
+"ON r.no_reg = ds.no_reg " 
...
+"AND ds.cont_no   = ?")            
    ...
    public Short getMasa3() {
        return masa3;
    }

    public void setMasa3(Short masa3) {
        this.masa3 = masa3;
    }
