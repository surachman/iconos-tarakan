...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByPpkb", query = 
"SELECT pu.id_uc " 
+"FROM uncontainerized_service pu " 
+"WHERE pu.no_ppkb         = ? " 
+"AND nvl(pu.is_delivery, 'FALSE') = 'FALSE'"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByPpkbAndBl", query = 
"SELECT pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
...
+"  m_port d, " 
+"  planning_vessel pv, " 
+"  preservice_vessel pl " 
+"WHERE nvl(pu.is_shifting, 'FALSE') = 'FALSE' " 
+"AND pu.commodity        = m.commodity_code " 
+"AND pu.operation        ='DISCHARGE' " 
+"AND pu.disch_port       =c.port_code " 
+"AND pu.load_port        =d.port_code " 
+"AND pu.no_ppkb          =pv.no_ppkb " 
+"AND pu.no_ppkb          = ? " 
+"AND pl.booking_code     =pv.booking_code " 
+"AND pl.next_port_code   = pu.disch_port"),   
...
*/                                                                                        
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByPpkbAndBlList", query = 
"SELECT a.bl_no, " 
+"  b.name, " 
+"  a.weight, " 
+"  c.name, " 
...
+"FROM uncontainerized_service a " 
+"JOIN m_commodity b " 
+"ON (a.commodity=b.commodity_code) " 
+"JOIN m_port c " 
+"ON (a.disch_port=c.port_code) " 
+"JOIN m_port d " 
+"ON (a.load_port       =d.port_code) " 
+"WHERE a.no_ppkb       = ? " 
+"AND nvl(a.is_shifting, 'FALSE') = 'TRUE'"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findForLiftOFF", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"WHERE pu.no_ppkb           = ? " 
+"AND pu.operation           = ? " 
+"AND nvl(pu.truck_loosing, 'FALSE') = 'FALSE' " 
+"AND pu.commodity           = m.commodity_code " 
+"AND pu.disch_port          = c.port_code " 
+"AND pu.load_port           = d.port_code"),    
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findLiftableOffByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"JOIN m_port c " 
+"ON (pu.disch_port = c.port_code) " 
+"JOIN m_port d " 
+"ON (pu.load_port = d.port_code) " 
+"LEFT JOIN uc_shifting_service uss " 
+"ON (pu.bl_no               = uss.bl_no " 
+"AND pu.no_ppkb             = uss.no_ppkb) " 
+"WHERE pu.no_ppkb           = ? " 
+"AND pu.status              = 'STV' " 
+"AND nvl(pu.truck_loosing, 'FALSE') = 'FALSE' " 
+"AND ((nvl(is_shifting, 'FALSE') = 'FALSE' " 
+"AND pu.operation           = 'DISCHARGE') " 
+"OR (nvl(pu.is_shifting, 'FALSE')   = 'FALSE'  " 
+"AND pu.shift_to            = 'TOCY' " 
+"AND nvl(uss.is_landed, 'FALSE')    = 'FALSE'))"),
/*
...
*/                                                                                            
    @NamedNativeQuery(name = "UncontainerizedService.Native.findHasLiftedOffByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findReceivableUcsByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"ON (pu.id_uc=urs.id_uc) " 
+"JOIN uc_gatereceiving_service ugs " 
+"ON (ugs.jobslip            =urs.jobslip) " 
+"WHERE pu.no_ppkb           = ? " 
+"AND pu.status              = 'PLANNING' " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByEntryDelivery", query = 
"SELECT pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
+"  change(pu.truck_loosing), " 
...
+"AND pu.no_ppkb                      = ? " 
+"AND pu.operation                    = 'DISCHARGE' " 
+"AND (NVL(pu.truck_loosing, 'FALSE') = 'TRUE' " 
+"OR (NVL(pu.truck_loosing, 'FALSE')  = 'FALSE' " 
+"AND pu.status                       = 'CY' " 
+"AND NVL(pu.is_delivery, 'FALSE')    = 'FALSE')) " 
+"AND pu.id_uc NOT                   IN " 
+"  (SELECT id_uc FROM uc_delivery_service WHERE id_uc = pu.id_uc " 
+"  ) " 
+"AND NVL(is_transhipment, 'FALSE')= 'FALSE'"),    
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByEntryReceivingUcLoad", query = 
"SELECT pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
+"  change(pu.truck_loosing), " 
...
+"AND pu.no_ppkb                       = ? " 
+"AND pu.operation                     = 'LOAD' " 
+"AND NVL(pu.is_transhipment, 'FALSE') = 'FALSE' " 
+"AND pu.id_uc NOT                    IN " 
+"  (SELECT id_uc FROM uc_receiving_service WHERE no_ppkb = pu.no_ppkb " 
+"  )"),
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDischargableUcsByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDischargedUcsByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"WHERE pu.no_ppkb                   = ? " 
+"AND (pu.status                     = 'STV' " 
+"OR (NVL(pu.truck_loosing, 'FALSE') = 'TRUE' " 
+"AND pu.status                      = 'CY' " 
+"AND NVL(pu.is_delivery, 'FALSE')   = 'FALSE' )) " 
+"AND ((NVL(is_shifting, 'FALSE')    = 'FALSE' " 
+"AND pu.operation                   = 'DISCHARGE') " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findLiftableOnByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"OR (NVL(pu.is_shifting, 'FALSE')   = 'FALSE' " 
+"AND pu.shift_to                    = 'TOCY' " 
+"AND NVL(uss.is_landed, 'FALSE')    = 'TRUE'))"),
/*
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findHasLiftedOnByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"AND pu.status                      = 'STV' " 
+"AND NVL(pu.truck_loosing, 'FALSE') = 'FALSE' " 
+"AND ((NVL(pu.is_shifting, 'FALSE') = 'FALSE' " 
+"AND pu.operation                   = 'LOAD') " 
+"OR (NVL(pu.is_shifting, 'FALSE')   = 'FALSE' " 
+"AND pu.shift_to                    = 'TOCY' " 
+"AND NVL(uss.is_landed, 'FALSE')    = 'TRUE'))"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findLoadableByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"AND ((NVL(pu.is_shifting, 'FALSE')  = 'FALSE' " 
+"AND pu.operation                    = 'LOAD') " 
+"OR (NVL(pu.is_shifting, 'FALSE')    = 'TRUE' " 
+"AND pu.shift_to                     = 'TOCY' " 
+"AND NVL(uss.is_landed, 'FALSE')     = 'TRUE' " 
+"AND NVL(uss.is_reshipping, 'FALSE') = 'FALSE'))"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findLoadableByNoPpkbAndNoBl", query = 
"SELECT pu.id_uc, " 
+"  pu.no_ppkb, " 
+"  pu.bl_no, " 
+"  c.name, " 
...
*/                                                                                            
    @NamedNativeQuery(name = "UncontainerizedService.Native.findHasLoadedByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"AND pu.no_ppkb                      = uss.no_ppkb) " 
+"WHERE pu.no_ppkb                    = ? " 
+"AND pu.status                       = 'VESSEL' " 
+"AND ((NVL(pu.is_shifting, 'FALSE')  = 'FALSE' " 
+"AND pu.operation                    = 'LOAD') " 
+"OR (NVL(pu.is_shifting, 'FALSE')    = 'TRUE' " 
+"AND pu.shift_to                     = 'TOCY' " 
+"AND NVL(uss.is_landed, 'FALSE')     = 'TRUE' " 
+"AND NVL(uss.is_reshipping, 'FALSE') = 'TRUE'))"),
  
    //penambahan nama kapal , vo in voy out by ade chelsea tanggal 17 maret 2014
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDetailByIdUc",  query = "SELECT a.bl_no, b.name, a.unit, a.weight, change(a.truck_loosing), c.name, d.name, a.block, kapal.no_ppkb,kapal.name,kapal.voy_in,kapal.voy_out "
            + "FROM uncontainerized_service a "
...
+ "WHERE p.status IN ('Approved', 'Servicing', 'ReadyService', 'Served') ) kapal on a.no_ppkb=kapal.no_ppkb "  
            + ", m_commodity b, m_port c, m_port d WHERE a.commodity=b.commodity_code AND a.disch_port=c.port_code AND a.load_port=d.port_code AND a.id_uc=?"),
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByPpkbAndBlMobile", query = "SELECT s.id_uc, s.no_ppkb, s.bl_no, c.name, s.weight, s.unit, s.truck_loosing, s.block FROM uncontainerized_service s, m_commodity c WHERE s.commodity = c.commodity_code AND s.no_ppkb = ? AND s.bl_no = ? AND s.status = ? AND s.operation = ? AND s.is_delivery = ?"),
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByBlNoMobile", query = "SELECT s.id_uc, s.no_ppkb, s.bl_no, c.name, s.weight, s.unit, s.truck_loosing, s.block FROM uncontainerized_service s, m_commodity c WHERE s.commodity = c.commodity_code AND s.bl_no = ? AND s.status = ? AND s.operation = ? AND s.is_delivery = ?"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDeliveredUcsByNoPpkb", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"WHERE pu.no_ppkb                   = ? " 
+"AND pu.operation                   = 'DISCHARGE' " 
+"AND NVL(pu.is_delivery, 'FALSE')   = 'TRUE' " 
+"AND (NVL(pu.truck_loosing, 'FALSE')= 'FALSE' " 
+"OR (NVL(pu.truck_loosing, 'FALSE') = 'TRUE' " 
+"AND pu.status                     IN ('OUTSIDE', 'CY')))"),
/*
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDeliverableUcs", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
+"JOIN m_port d " 
+"ON (pu.load_port = d.port_code) " 
+"JOIN uc_delivery_service uds " 
+"ON (pu.id_uc=uds.id_uc) " 
+"JOIN uc_gatedelivery_service ugs " 
+"ON (uds.jobslip          = ugs.jobslip) " 
+"WHERE pu.no_ppkb         = ? " 
+"AND pu.status            = 'CY' " 
+"AND pu.operation         = 'DISCHARGE' " 
+"AND nvl(pu.is_delivery, 'FALSE') = 'FALSE'"),
...
/*    
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDeliverableUcs2", query = "SELECT pu.id_uc, pu.bl_no, m.name, pu.unit, pu.weight, change(pu.truck_loosing), c.name, d.name, pu.block "
                                                                                            + "FROM uncontainerized_service pu "
                                                                                                    + "JOIN m_commodity m ON (pu.commodity = m.commodity_code) "
                                                                                                    + "JOIN m_port c ON (pu.disch_port = c.port_code) "
                                                                                                    + "JOIN m_port d ON (pu.load_port = d.port_code) "
                                                                                                    + "JOIN uc_delivery_service uds ON (pu.id_uc=uds.id_uc) "
                                                                                                            + "JOIN uc_gatedelivery_service ugs ON (uds.jobslip = ugs.jobslip) "
                                                                                            + "WHERE pu.status = 'CY' AND pu.operation = 'DISCHARGE' AND pu.is_delivery = FALSE"),
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDeliverableUcs2", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findDeliverableUcByBlNo", query = 
"SELECT pu.id_uc, " 
+"  pu.no_ppkb, " 
+"  pu.bl_no, " 
+"  m.name, " 
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findShifting", query = 
"SELECT s.id_uc, " 
+"  s.no_ppkb, " 
+"  s.bl_no, " 
+"  c.name, " 
+"  s.weight, " 
+"  s.unit " 
+"FROM uncontainerized_service s, " 
+"  m_commodity c " 
+"WHERE s.commodity       = c.commodity_code " 
+"AND s.no_ppkb           = ? " 
+"AND nvl(s.is_shifting, 'FALSE') = 'TRUE' " 
+"AND s.operation         = 'DISCHARGE' " 
+"AND s.status           != 'STV' " 
+"AND s.bl_no NOT        IN " 
+"  (SELECT bl_no FROM uc_shifting_service WHERE no_ppkb = s.no_ppkb " 
+"  )"),    
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findShiftingWithout", query = 
"SELECT s.id_uc, " 
+"  s.no_ppkb, " 
+"  s.bl_no, " 
+"  c.name, " 
...
+"WHERE s.commodity       = c.commodity_code " 
+"AND s.no_ppkb           = ? " 
+"AND nvl(s.is_shifting, 'FALSE') = 'FALSE' " 
+"AND s.bl_no NOT        IN " 
+"  (SELECT bl_no FROM uc_shifting_service WHERE no_ppkb=s.no_ppkb " 
+"  )"),
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findByBlNoMobileUc", query = "SELECT s.id_uc, s.no_ppkb, s.bl_no, c.name, s.weight, s.unit, s.truck_loosing, s.block FROM uncontainerized_service s, m_commodity c WHERE s.commodity = c.commodity_code AND s.id_uc IN (select id_uc from uc_receiving_service) AND s.status IN ('PLANNING', 'UNPLANNED') AND s.bl_no = ?"),
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findShiftingWithoutMobile", query = 
"SELECT s.id_uc, " 
+"  s.no_ppkb, " 
+"  s.bl_no, " 
+"  c.name, " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findShiftingMobile", query = 
"SELECT s.id_uc, " 
+"  s.no_ppkb, " 
+"  s.bl_no, " 
+"  c.name, " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findForLoadConfirmByTL", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.unit, " 
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findUncontainerizedTranshipment", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
...
+"  pu.unit, " 
+"  pu.block, " 
+"  mp1.name AS loadPort, " 
+"  mp2.name AS dischPort, " 
+"  pu.status, " 
+"  pu.operation " 
+"FROM uncontainerized_service pu, " 
+"  m_commodity m, " 
...
+"  m_port mp2 " 
+"WHERE pu.commodity = m.commodity_code " 
+"AND mp1.port_code  =load_port " 
+"AND mp2.port_code  =disch_port " 
+"AND pu.disch_port != " 
+"  (SELECT value_string " 
+"  FROM m_setting_app " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findUncontainerizedAfterTranshipment", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
...
+"  ) " 
+"AND pu.operation          ='DISCHARGE' " 
+"AND nvl(is_transhipment, 'FALSE') = 'TRUE' " 
+"AND pu.no_ppkb            = ?"),
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findUncontainerizedBayPlanLoadListSelect", query = 
"SELECT pu.id_uc, " 
+"  pu.bl_no, " 
+"  m.name, " 
+"  pu.weight, " 
...
+"  m_port mp2 " 
+"WHERE pu.commodity           = m.commodity_code " 
+"AND mp1.port_code            =pu.load_port " 
+"AND mp2.port_code            =pu.disch_port " 
+"AND nvl(pu.is_transhipment, 'FALSE') = 'TRUE' " 
+"AND (pu.new_ppkb             = ? " 
+"OR pu.new_ppkb              IS NULL) " 
+"AND pu.status                ='CY' " 
+"AND nvl(pu.bay_plan, 'FALSE')   = 'FALSE' " 
+"AND pu.bl_no NOT            IN " 
+"  (SELECT u.bl_no " 
+"  FROM uncontainerized_service u " 
+"  WHERE pu.no_ppkb          =u.no_ppkb " 
+"  AND pu.bl_no              =u.bl_no " 
+"  AND operation             ='LOAD' " 
+"  AND nvl(is_transhipment, 'FALSE') = 'FALSE' " 
+"  )"),    
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findHandlingDischarge", query = 
"SELECT us.bl_no, " 
+"  (SELECT mc.name FROM m_commodity mc WHERE us.commodity = mc.commodity_code " 
+"  ) AS commodity, " 
+"  us.unit, " 
+"  us.weight, " 
...
+"AND nvl(us.is_shifting, 'FALSE')  = 'FALSE' " 
+"AND us.no_ppkb            = ?"),  
/*                                                                                          
...
*/
    ...
    @NamedNativeQuery(name = "UncontainerizedService.Native.findHandlingLoad", query = 
"SELECT us.bl_no, " 
+"  (SELECT mc.name FROM m_commodity mc WHERE us.commodity = mc.commodity_code " 
+"  ) AS commodity, " 
+"  us.unit, " 
...
+"  e1.end_time   AS lift_date, " 
+"  e2.equip_code AS ht_code, " 
+"  e2.end_time   AS ht_date, " 
+"  e3.equip_code AS cc_code, " 
+"  e3.end_time   AS stv_date " 
+"FROM uncontainerized_service us " 
+"LEFT JOIN equipment e1 " 
+"ON e1.no_ppkb             = us.no_ppkb " 
+"AND e1.bl_no              = us.bl_no " 
...
*/
    @NamedNativeQuery(name = "UncontainerizedService.Native.findUncontainerizedBayPlanLoadId", query = 
"SELECT id_uc " 
+"FROM " 
+"  (SELECT pu.id_uc " 
+"  FROM uncontainerized_service pu " 
+"  WHERE pu.operation           ='DISCHARGE' " 
+"  AND nvl(pu.is_transhipment, 'FALSE') = 'TRUE' " 
+"  AND pu.no_ppkb               = ? " 
+"  AND pu.bl_no                 = ? " 
+"  ) " 
...
public class UncontainerizedService implements Serializable, EntityAuditable {
    ...
    private String truckLoosing = "FALSE";
    ...
    public String getTruckLoosing() {
        return truckLoosing;
    }

    public void setTruckLoosing(String truckLoosing) {
