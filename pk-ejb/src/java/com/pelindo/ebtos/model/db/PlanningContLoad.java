...
@NamedQueries({
    ...
    @NamedQuery(name = "PlanningContLoad.findByNoPpkb", query = "SELECT p FROM PlanningContLoad p WHERE p.planningVessel.noPpkb = :noPpkb"),
    @NamedQuery(name = "PlanningContLoad.findByPpkbNotCompleted", query = "SELECT p FROM PlanningContLoad p WHERE p.completed = FALSE AND p.planningVessel.noPpkb = :noPpkb")
})
@NamedNativeQueries({
...
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningBayplanLoadsByPpkb", query = 
"SELECT pcl.cont_no, " 
+"  pcl.cont_size, " 
+"  mct.name, " 
+"  pcl.cont_status, " 
...
+"  pcl.bl_no, " 
+"  CASE " 
+"    WHEN pcl.no_ppkb = rs.no_ppkb " 
+"    AND pcl.cont_no  =rs.cont_no " 
+"    THEN " 
+"      (SELECT mp.name " 
+"      FROM m_port mp " 
+"      WHERE mp.port_code=rs.disch_port_code " 
+"      AND rownum        < 2 " 
+"      ) " 
+"    WHEN pcl.no_ppkb=st.new_ppkb " 
+"    AND pcl.cont_no =st.cont_no " 
+"    THEN " 
+"      (SELECT mp.name " 
+"      FROM m_port mp " 
+"      WHERE mp.port_code=st.disch_port " 
+"      AND rownum        < 2 " 
+"      ) " 
+"  END " 
...
+"AND pcl.cont_no=rs.cont_no) " 
+"LEFT JOIN service_cont_transhipment st " 
+"ON pcl.no_ppkb =st.new_ppkb " 
+"AND pcl.cont_no=st.cont_no), " 
...
+"ORDER BY pcl.id DESC"),
    ...
    @NamedNativeQuery(name = "PlanningContLoad.Native.findLoadCandidatesPerCustomer", query = 
"SELECT pcl.cont_no, " 
+"  mc.name, " 
+"  pcl.cont_size, " 
+"  pcl.cont_status, " 
+"  mct.type_in_general, " 
+"  pcl.block, " 
+"  pcl.created_date placement_date, " 
+"  mv.name " 
+"FROM planning_cont_load pcl " 
...
+"WHERE scl.cont_no             IS NULL " 
+"AND pcl.position               = '" +PlanningContLoad.LOCATION_CY + "' " 
+"AND pcl.status_cancel_loading <> 'TRUE' " 
+"AND mc.cust_code               = ? " 
+"ORDER BY pcl.created_date ASC"),

    @NamedNativeQuery(name = "PlanningContLoad.Native.findLoadCandidatesPerCustomer_summary", query = 
"SELECT mc.cust_code, " 
+"  COUNT( " 
+"  CASE " 
+"    WHEN pcl.cont_status = 'MTY' " 
+"    AND pcl.cont_size    = 40 " 
+"    THEN 1 " 
+"  END), " 
+"  COUNT( " 
+"  CASE " 
+"    WHEN pcl.cont_status = 'MTY' " 
+"    AND pcl.cont_size    = 20 " 
+"    THEN 1 " 
+"  END), " 
+"  COUNT( " 
+"  CASE " 
+"    WHEN pcl.cont_status = 'FCL' " 
+"    AND pcl.cont_size    = 40 " 
+"    THEN 1 " 
+"  END), " 
+"  COUNT( " 
...
+"GROUP BY mc.cust_code"),
    ...
    @NamedNativeQuery(name = "PlanningContLoad.Native.findCountableLoadReefers", query = 
"SELECT NULL, " 
+"  pcl.cont_no, " 
+"  pcl.cont_size, " 
+"  mt.name, " 
...
+"ON (pcl.cont_no=sr.cont_no " 
+"AND pcl.no_ppkb=sr.no_ppkb) " 
+"LEFT JOIN reefer_load_service rls " 
+"ON (pcl.cont_no=rls.cont_no " 
+"AND pcl.no_ppkb=rls.no_ppkb) " 
+"LEFT JOIN " 
+"  (SELECT rls.cont_no, " 
+"    rls.no_ppkb, " 
+"    rls.no_reg " 
...
+"  JOIN registration r " 
+"  ON (rls.no_reg          =r.no_reg ) " 
+"  WHERE r.status_service IN ('registrasi','confirm') " 
+"  ) rls2 ON (rls.no_ppkb  = rls2.no_ppkb " 
+"AND rls.cont_no           = rls2.cont_no) " 
+"JOIN m_container_type mt " 
+"ON (pcl.cont_type       =mt.cont_type) " 
+"WHERE rls2.cont_no     IS NULL " 
+"AND (pcl.is_transhipment='TRUE' " 
+"OR pcl.is_shifting      ='TRUE' " 
+"OR pcl.is_change_vessel ='TRUE' " 
+"OR (sr.plug_off         > (sr.plug_on + rls.shift_planning * 8 * (interval '1' Hour)))) " 
+"AND sr.plug_off        IS NOT NULL " 
+"AND mt.type_in_general  = 'RFR' " 
+"AND pcl.no_ppkb         = ? " 
+"GROUP BY pcl.cont_no, " 
+"  pcl.cont_size, " 
+"  mt.name, " 
+"  pcl.cont_status, " 
+"  pcl.cont_gross, " 
...
+"  pcl.is_transhipment, " 
+"  pcl.is_shifting, " 
+"  pcl.is_change_vessel, " 
+"  sr.plug_on, " 
+"  rls.shift_planning"),

    @NamedNativeQuery(name = "PlanningContLoad.Native.findPluggableOnContainersByPpkb", query = 
"SELECT pcl.cont_no, " 
+"  pcl.cont_size, " 
+"  mct.name, " 
+"  pcl.cont_status, " 
...
+"JOIN m_container_type mct " 
+"ON (pcl.cont_type = mct.cont_type) " 
+"LEFT JOIN service_reefer pclf " 
+"ON (pclf.no_ppkb               = pcl.no_ppkb " 
+"AND pclf.cont_no               = pcl.cont_no) " 
+"WHERE mct.type_in_general      = 'RFR' " 
+"AND pclf.cont_no              IS NULL " 
+"AND pcl.position               = '03' " 
+"AND pcl.status_cancel_loading <> 'TRUE' " 
+"AND pcl.no_ppkb                = ? " 
+"ORDER BY pcl.id DESC"),

    @NamedNativeQuery(name = "PlanningContLoad.Native.findPluggableContainer", query = "SELECT pcl.cont_no, " 
+"  pcl.cont_size, " 
+"  pcl.cont_type, " 
+"  pcl.cont_status, " 
+"  pcl.cont_gross, " 
...
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPodByNoPpkbAndYardLocation", query = 
"SELECT DISTINCT pcl.disch_port " 
+"FROM planning_cont_load pcl " 
+"WHERE pcl.no_ppkb             = ? " 
+"AND pcl.block                 = ? " 
+"AND pcl.y_slot                = ? " 
+"AND pcl.completed             = 'FALSE' " 
+"AND pcl.status_cancel_loading = 'FALSE' " 
+"ORDER BY pcl.disch_port"),
/*
    @NamedNativeQuery(name = "PlanningContLoad.Native.findLoadingListByPpkb", query = "SELECT loading.cont_no, " 
																								+"  loading.cont_size, " 
																								+"  loading.cont_type, " 
																								+"  loading.cont_status, " 
																								+"  loading.cont_gross, " 
																								...
																								+"    mp.name, " 
																								+"    CASE " 
																								+"      WHEN pcl.is_transhipment = 1 " 
																								+"      THEN 'TRANSHIPMENT' " 
																								+"      WHEN pcl.is_change_vessel = 1 " 
																								+"      THEN 'EX CANCEL LOADING' " 
																								+"      WHEN pcl.is_shifting = 1 " 
																								+"      THEN 'SHIFTING' " 
																								+"      ELSE 'RECEIVING' " 
																								+"    END AS status, " 
																								+"    CASE " 
																								...
																								+"      THEN mc_cls.name " 
																								+"      WHEN pcl.is_transhipment = 1 " 
																								+"      OR pcl.is_shifting       = 1 " 
																								+"      THEN mc_vs.name " 
																								+"      ELSE mc.name " 
																								+"    END AS cust_name, " 
																								+"    CASE " 
																								+"      WHEN (sgr.cont_no      IS NOT NULL) " 
																								+"      OR pcl.is_change_vessel = 1 " 
																								+"      OR pcl.is_transhipment  = 1 " 
																								+"      OR pcl.is_shifting      = 1 " 
																								+"      THEN 'OS' " 
																								+"      ELSE 'GT' " 
																								+"    END AS i_stat, " 
																								...
																								+"  ) loading " 
																								+"WHERE loading.no_ppkb                 = ? " 
																								+"AND NOT loading.status_cancel_loading = 1 " 
																								...
																								+"  loading.y_tier ASC;"),
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findLoadingListByPpkb", query = 
...
+"  ON (pcl.no_ppkb              =rs.no_ppkb " 
+"  AND pcl.cont_no              =rs.cont_no " 
+"  AND rs.status_cancel_loading = 'FALSE') " 
+"  LEFT JOIN registration r " 
+"  ON (r.no_reg=rs.no_reg) " 
+"  LEFT JOIN m_customer mc " 
+"  ON (r.cust_code=mc.cust_code) " 
...
+"  loading.y_row, " 
+"  loading.y_tier ASC"),																								
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningBayplanLoadCekBayExist", query = 
"SELECT pcl.id, " 
+"  pcl.v_bay, " 
+"  pcl.v_row, " 
+"  pcl.v_tier, " 
...
+"  pcl.no_ppkb " 
+"FROM planning_cont_load pcl, " 
+"  (SELECT ? bay,? \"row\",? tier FROM dual " 
+"  ) prm " 
+"WHERE (pcl.v_bay  =prm.bay " 
+"OR (pcl.cont_size = 40 " 
+"AND pcl.v_bay     = prm.bay - 2)) " 
+"AND pcl.v_row     =prm.\"row\" " 
+"AND pcl.v_tier    =prm.tier " 
+"AND pcl.no_ppkb   =?;"),
...
/*    
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningContLoads3", query = "SELECT r.cont_no, r.cont_size, mt.name, r.cont_status, r.cont_gross, r.seal_no,r.commodity_code,r.over_size,r.dg,r.id,r.block,r.y_slot,r.y_row,r.y_tier,r.disch_port "
                                                                                    ...
                                                                                    + "WHERE r.position='" + PlanningContLoad.LOCATION_CY + "' AND r.status_cancel_loading <> 1 AND r.is_shifting <> 1 AND r.no_ppkb= ? "
                                                                                    + "ORDER BY r.id DESC;"),
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningContLoads3", query = 
"SELECT r.cont_no, " 
+"  r.cont_size, " 
+"  mt.name, " 
+"  r.cont_status, " 
...
+"JOIN m_yard my " 
+"ON (my.block                 =r.block) " 
+"WHERE r.position             ='" + PlanningContLoad.LOCATION_CY + "' " 
+"AND r.status_cancel_loading <> 'TRUE' " 
+"AND r.is_shifting           <> 'TRUE' " 
+"AND r.no_ppkb                = ? " 
+"ORDER BY r.id DESC"),

/*                                                                                    
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningContLoadsTranshipment", query = "SELECT r.cont_no, r.cont_size, mt.name, r.cont_status, r.cont_gross, r.seal_no,r.commodity_code,r.over_size,r.dg,r.id,r.block,r.y_slot,r.y_row,r.y_tier,r.disch_port from planning_cont_load r,m_container_type mt,m_yard my Where r.is_transhipment =1 AND r.position='" + PlanningContLoad.LOCATION_CY + "' AND r.cont_type=mt.cont_type AND r.status_cancel_loading=0 AND r.no_ppkb= ? AND my.block=r.block ORDER BY r.id DESC"),
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlanningContLoadsTranshipment", query = 
"SELECT r.cont_no, " 
+"  r.cont_size, " 
+"  mt.name, " 
+"  r.cont_status, " 
...
+"  m_container_type mt, " 
+"  m_yard my " 
+"WHERE r.is_transhipment    = 'TRUE' " 
+"AND r.position             ='\" + PlanningContLoad.LOCATION_CY + \"' " 
+"AND r.cont_type            =mt.cont_type " 
+"AND r.status_cancel_loading='FALSE' " 
+"AND r.no_ppkb              = ? " 
+"AND my.block               =r.block " 
+"ORDER BY r.id DESC"),    
    ...
    @NamedNativeQuery(name = "PlanningContLoad.Native.findLiftableOnContainer", query = "SELECT pcl.cont_no, pcl.no_ppkb, mv.name vessel_name, pcl.block, pcl.y_slot, pcl.y_row, pcl.y_tier, pcl.cont_size, pcl.cont_status, pcl.over_size, pcl.dg, pcl.is_transhipment, pcl.is_shifting IS NOT NULL AND pcl.is_shifting AS is_shifting, CASE WHEN sgr.cont_no IS NOT NULL THEN mcd_gt.name WHEN sct.cont_no IS NOT NULL THEN mcd_d.name ELSE mcd.name END "
                                                                                        + "FROM planning_cont_load pcl "
                                                                                                + "JOIN (planning_vessel pv "
                                                                                                        ...
                                                                                                        + "LEFT JOIN receiving_service rs ON (pcl.cont_no=rs.cont_no AND pcl.no_ppkb=rs.no_ppkb AND rs.status_cancel_loading <> 1) "
                                                                                                                + "LEFT JOIN (service_gate_receiving sgr "
                                                                                        ...
                                                                                        + "WHERE pcl.cont_no = ? AND pcl.position = '" + PlanningContLoad.LOCATION_CY + "' AND pcl.status_cancel_loading = 0;"),
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findLiftableOnContainer", query = 
"SELECT pcl.cont_no, " 
+"  pcl.no_ppkb, " 
+"  mv.name vessel_name, " 
+"  pcl.block, " 
...
+"LEFT JOIN (service_gate_receiving sgr " 
+"LEFT JOIN m_cont_damage mcd_gt " 
+"ON (sgr.cont_damage=mcd_gt.id)) " 
+"ON (rs.job_slip    =sgr.job_slip) " 
+"LEFT JOIN service_cont_transhipment sct " 
+"ON (pcl.cont_no=sct.cont_no " 
+"AND pcl.no_ppkb=sct.new_ppkb) " 
+"LEFT JOIN (service_cont_discharge scd " 
+"LEFT JOIN m_cont_damage mcd_d " 
+"ON (scd.cont_damage=mcd_d.id)) " 
+"ON (sct.cont_no    =scd.cont_no " 
+"AND sct.no_ppkb    =scd.no_ppkb) " 
+"LEFT JOIN service_shifting ss " 
+"ON (pcl.cont_no=ss.cont_no " 
+"AND pcl.no_ppkb=ss.no_ppkb) " 
+"LEFT JOIN m_cont_damage mcd " 
+"ON (ss.cont_damage            =mcd.id) " 
+"WHERE pcl.cont_no             = ? " 
+"AND pcl.position              = '" + PlanningContLoad.LOCATION_CY + "' " 
+"AND pcl.status_cancel_loading = 'FALSE'"),
...
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findPlannedContainerByBayLocation", query = 
"SELECT p.cont_no, " 
+"  mct.new_iso_code, " 
+"  p.cont_gross, " 
+"  p.v_bay, " 
...
+"FROM planning_cont_load p " 
+"JOIN m_container_type mct " 
+"ON (p.cont_type=mct.cont_type), " 
+"  (SELECT ? AS bay FROM DUAL " 
+"  ) obj " 
+"WHERE p.no_ppkb  = ? " 
+"AND (p.v_bay     = obj.bay " 
+"OR (p.v_bay      = obj.bay - 2 " 
+"AND p.cont_size           IN (40, 45)) " 
+"OR (p.v_bay      = obj.bay - 4 " 
+"AND p.cont_size  = 45)) " 
+"AND p.v_row     IS NOT NULL " 
+"AND p.v_tier    IS NOT NULL"),
...
/*
    ...
    + "ORDER BY row_number;"),
*/
    @NamedNativeQuery(name = "PlanningContLoad.Native.findGrossCapacityByNoPpkbAndBay", query = 
"SELECT column_value , " 
+"  NVLN(pcl.gross) AS gross " 
+"FROM TABLE(generate_series( ?, ?)) row_number " 
+"LEFT JOIN " 
+"  (SELECT pcl.v_row, " 
+"    SUM(pcl.cont_gross) AS gross " 
+"  FROM planning_cont_load pcl, " 
+"    (SELECT ? AS bay FROM DUAL " 
+"    ) obj " 
+"  WHERE (pcl.v_bay  = obj.bay " 
+"  OR (pcl.v_bay     = obj.bay - 2 " 
+"  AND pcl.cont_size = 40)) " 
+"  AND pcl.no_ppkb   = ? " 
+"  AND pcl.v_tier BETWEEN ? AND ? " 
+"  GROUP BY pcl.v_row " 
+"  ) pcl " 
+"ON (row_number.column_value=pcl.v_row) " 
+"ORDER BY row_number.column_value"),
    
    @NamedNativeQuery(name = "PlanningContLoad.Native.PlanningContLoadfindCancelLoading", query = "SELECT scd.id,scd.cont_no FROM planning_cont_load scd WHERE scd.no_ppkb = ? AND scd.cont_no = ? AND scd.cont_no NOT IN (select cont_no from cancel_loading_service where no_ppkb=scd.no_ppkb)"),
    
    @NamedNativeQuery(name ="PlanningContLoad.Native.findContainerStatus",query="SELECT container.container, " 
...
public class PlanningContLoad implements Serializable, EntityAuditable {
    ...
    public String getIsSling() {
        return isSling;
    }

    public void setIsSling(String isSling) {
