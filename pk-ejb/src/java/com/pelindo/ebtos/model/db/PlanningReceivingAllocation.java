...
@NamedQueries({
    ...
    @NamedQuery(name = "PlanningReceivingAllocation.findByNoPpkbAndContainer", query="SELECT p FROM PlanningReceivingAllocation p WHERE p.planningVessel.noPpkb = :noPpkb AND p.contSize = :contSize and p.masterContainerType.contType = :contType and p.contStatus = :contStatus ")
})
@NamedNativeQueries({
/*
...
// No such fields as r.block, r.fr_slot, r.to_slot, r.fr_row, r.to_row in table planning_receiving_allocation
*/
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findPlanningReceivingAllocationsByBooking", query =
"SELECT r.cont_size, " 
+"  c.name, " 
+"  r.cont_status, " 
+"  r.gross_class, " 
...
+"  m_container_type c " 
+"WHERE c.cont_type=r.cont_type " 
+"AND r.no_ppkb    = ? " 
+"ORDER BY r.id DESC"),

    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findPlanningReceivingAllocationsByMapping", query = "SELECT r.cont_size, c.name,r.cont_status, r.gross_class, r.over_size, r.dg, pp.block, pp.fr_slot, pp.to_slot, pp.fr_row, pp.to_row, r.id FROM planning_receiving_allocation r, m_container_type c ,planning_receiving pp WHERE c.cont_type=r.cont_type AND r.id=pp.receiving_allocation_id AND r.no_ppkb = ? ORDER BY r.id DESC"),

// Query dibawah Error tidak ada kolom "block" di table planning_receiving_allocation
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findPlanningReceivingAllocationsByBlock", query = "SELECT r.id, r.block FROM planning_receiving_allocation r WHERE r.block = ?"),
    ...
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findPlanningReceivingList", query = "SELECT r.id, r.block,r.fr_slot,r.to_slot,r.fr_row,r.to_row FROM planning_receiving r, planning_receiving_allocation p WHERE r.receiving_allocation_id=p.id AND r.receiving_allocation_id=? ORDER BY r.id DESC"),
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findPlanningReceivingSearchId", query = "SELECT p.id,p.no_ppkb FROM planning_receiving_allocation p WHERE p.no_ppkb= ? ORDER BY p.id DESC"),
...
*/
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findAllByPPKB", query = 
"SELECT b.name, " 
+"  a.cont_size, " 
+"  a.cont_status, " 
+"  c.description, " 
...
+"  CASE " 
+"    WHEN (SELECT id " 
+"      FROM planning_cont_receiving pr " 
+"      WHERE pr.no_ppkb  =a.no_ppkb " 
+"      AND pr.cont_size  =a.cont_size " 
+"      AND pr.cont_status=a.cont_status " 
+"      AND pr.gross_class=a.gross_class " 
+"      AND pr.cont_type  =a.cont_type " 
+"      AND pr.dg         =a.dg " 
+"      AND pr.over_size  =a.over_size " 
+"      AND pr.disch_port =a.port_code " 
+"      AND pr.is_export  =a.is_export " 
...
+"  CASE " 
+"    WHEN a.cont_size       = 40 " 
+"    AND NVLN(pra.realized) > 0 " 
+"    THEN NVLN(pra.realized) / 2 " 
+"    ELSE NVLN(pra.realized) " 
+"  END AS realized, " 
+"  a.total_booking2, " 
+"  a.is_export, " 
+"  b.type_in_general " 
...
+"    ) pya ON (myc.no_ppkb         = pya.no_ppkb " 
...
+"  m_gross_class c, " 
+"  m_port e " 
+"WHERE a.cont_type=b.cont_type " 
+"AND a.gross_class=c.gross_class " 
+"AND a.port_code  =e.port_code " 
+"AND a.no_ppkb    = ? " 
+"ORDER BY a.id DESC"),
    ...
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findValidasiConstraint", query = "SELECT r.id FROM planning_receiving_allocation r where r.no_ppkb= ? AND r.cont_size=? and r.cont_type=? and r.cont_status=? and r.over_size=? and r.dg=? and r.gross_class=? AND r.port_code=? AND r.is_export=? AND ROWNUM < 2"),
    ...
    @NamedNativeQuery(name = "PlanningReceivingAllocation.Native.findAllocationBooking", query = "SELECT sum(total_booking)  FROM planning_receiving_allocation WHERE no_ppkb=?")})
