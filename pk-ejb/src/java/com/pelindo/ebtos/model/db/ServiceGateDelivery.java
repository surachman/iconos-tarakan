...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findServiceGateDeliveryDateOutByJobSlip", query = "SELECT r.id,r.job_slip,r.cont_no,m.iso_code,r.no_ppkb,r.seal_no FROM service_gate_delivery r,m_container_type m WHERE r.cont_type=m.cont_type AND r.cont_no = ? AND r.status='N' AND r.cont_no NOT IN (SELECT cont_no FROM service_delivery WHERE no_ppkb = r.no_ppkb)"),
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findGateOutPassableJobslips", query = "SELECT d.job_slip "
                                                                                                        ...
                                                                                                        + "WHERE d.job_slip LIKE ('%'|| ? ||'%') AND d.date_out IS NULL AND sd.is_delivery = 'TRUE' AND ROWNUM <= 10 "
                                                                                                        + "ORDER BY SUBSTRING(d.job_slip,7,5) DESC "),
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findGateOutPassableJobslips2", query = "SELECT d.cont_no "
                                                                                                        + "FROM service_gate_delivery d "
                                                                                                                + "JOIN (SELECT cont_no, no_ppkb, is_delivery FROM service_cont_discharge "
                                                                                                                        + "UNION ALL "
                                                                                                                        + "SELECT cont_no, no_ppkb, is_delivery FROM cancel_loading_service) sd ON (d.cont_no=sd.cont_no AND d.no_ppkb=sd.no_ppkb) "
                                                                                                        + "WHERE d.cont_no LIKE ('%'|| ? ||'%') AND d.date_out IS NULL AND ROWNUM <= 10"),
// menampilkan data container langsung ke gateout tanpa lewat yard operation - delivery confirm           + "AND sd.is_delivery = TRUE "                                                                                                        
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findGateOutPassableByJobSlip", query = "SELECT r.id, r.job_slip, r.cont_no, m.iso_code, r.no_ppkb, r.seal_no, sd.service_type "
                                                                                                                + "FROM service_gate_delivery r "
                                                                                                                        ...
                                                                                                                        + "JOIN (SELECT cont_no, no_ppkb, is_delivery, 'DISCHARGE' service_type FROM service_cont_discharge "
                                                                                                                                + "UNION ALL "
    ...
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findGateOutPassableByJobSlip2", query = "SELECT r.id, r.job_slip, r.cont_no, m.iso_code, r.no_ppkb, r.seal_no, sd.service_type "
                                                                                                                + "FROM service_gate_delivery r "
                                                                                                                        + "JOIN m_container_type m ON (r.cont_type=m.cont_type) "
                                                                                                                        + "JOIN (SELECT cont_no, no_ppkb, is_delivery, 'DISCHARGE' service_type FROM service_cont_discharge "
                                                                                                                                + "UNION ALL "
...
// menampilkan data container langsung ke gateout tanpa lewat yard operation - delivery confirm           + "AND sd.is_delivery = TRUE "
    ...
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.gateMonitoringPerCustomer", query = "SELECT sgd.cont_no, mc.name shipping, mct.feet_mark, mct.type_in_general, sgd.cont_weight, sgd.date_out date_out_delivery, CASE WHEN ss.cont_no IS NOT NULL THEN 'STRIPPING'::varchar ELSE 'DELIVERY' END service, scd.cont_status, sgd.vehicle_code, mv.name vessel_name, prv.voy_in, prv.voy_out, sgr.date_in date_in_stripping, scd.start_placement_date, mc_rd.name "
            ...
            + "WHERE sgd.date_in BETWEEN ? AND ? AND mc.cust_code = ? "
    ...
    @NamedNativeQuery(name = "ServiceGateDelivery.Native.findGateInPassableJobSlips", query = "SELECT t.cont_no\n" +
            "FROM\n" +
            "(SELECT d.job_slip, d.created_date,d.cont_no\n" +
            "	FROM delivery_service d \n" +
            "		LEFT JOIN service_gate_delivery sgd ON (d.job_slip = sgd.job_slip)\n" +
            "	WHERE sgd.job_slip IS NULL\n" +
            "	UNION ALL\n" +
            "	SELECT scd.job_slip, scd.created_date,scd.cont_no\n" +
            ...
            "WHERE t.cont_no LIKE ('%'|| ? ||'%') AND ROWNUM <= 10 \n" +
