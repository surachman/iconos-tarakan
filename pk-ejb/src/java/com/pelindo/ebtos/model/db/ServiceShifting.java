...
/*
    @NamedNativeQuery(name = "ServiceShifting.Native.findServiceShiftingReshippingFalse", query = "SELECT ss.id, ss.cont_no, ss.cont_size, c.name, ss.cont_status, ss.cont_gross, ss.seal_no, ss.shift_to, ss.v_bay, ss.v_row, ss.v_tier "
                                                                                                ...
                                                                                                + "ORDER BY ss.id DESC"),
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findServiceShiftingReshippingFalse", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.cont_size, " 
+"  c.name, " 
...
+"ON (ss.cont_type = c.cont_type) " 
+"LEFT JOIN equipment e " 
+"ON (e.no_ppkb = ss.no_ppkb " 
+"AND e.cont_no = ss.cont_no " 
+"AND (e.operation LIKE 'SHIFTING%' " 
+"OR e.operation       = 'RESHIPPING')) " 
+"WHERE ss.no_ppkb     = ? " 
+"AND ss.is_reshipping = 'FALSE' " 
+"AND ss.is_planning   = 'TRUE' " 
+"AND e.cont_no       IS NULL " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findShiftableContainer", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.bl_no, " 
+"  ss.cont_gross, " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findServiceShiftingReshipping", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.cont_size, " 
...
+"  ss.is_reshipping, " 
+"  ss.id_equipment, " 
+"  ss.id_equipment_reshipping " 
+"FROM service_shifting ss, " 
+"  m_container_type c " 
+"WHERE ss.cont_type    = c.cont_type " 
+"AND ss.no_ppkb        = ? " 
+"AND (ss.is_reshipping = 'TRUE' " 
+"OR ss.is_landed       = 'TRUE') " 
+"AND ss.is_planning    = 'TRUE' " 
+"ORDER BY ss.id DESC"),
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findServiceShiftingReshippingNotToCY", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.cont_size, " 
+"  c.type_in_general AS name, " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findServiceShiftingReshippingWithout", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.cont_size, " 
+"  c.type_in_general AS name, " 
...
+"AND ss.is_reshipping = 'FALSE' " 
+"AND ss.is_planning   = 'TRUE' " 
+"AND ss.cont_no NOT  IN " 
+"  (SELECT e.cont_no " 
+"  FROM equipment e " 
+"  WHERE e.no_ppkb = ss.no_ppkb " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findByContNoMobile", query = 
"SELECT ss.id, " 
+"  ss.no_ppkb, " 
+"  ss.cont_no, " 
+"  ss.v_bay, " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findRekap", query = 
"SELECT ss.id, " 
+"  ss.cont_no, " 
+"  ss.cont_size, " 
...
+"  er.end_time " 
+"FROM service_shifting ss " 
+"LEFT JOIN equipment er " 
+"ON ss.id_equipment_reshipping = er.id, " 
+"  m_container_type ct, " 
+"  equipment e " 
+"WHERE ss.cont_type  = ct.cont_type " 
+"AND ss.id_equipment = e.id " 
+"AND ss.is_paid      = 'TRUE' " 
+"AND ss.no_ppkb      = ? " 
+"AND ss.operation    = ?"),
...
+"  'FALSE' AS topTier, " 
+"  scd.id " 
+"FROM service_shifting scd, " 
+"  m_container_type mct, " 
+"  planning_shift_discharge ps " 
+"WHERE scd.cont_type = mct.cont_type " 
+"AND scd.position    ='03' " 
+"AND scd.no_ppkb     = ? " 
+"AND ps.no_ppkb      =scd.no_ppkb " 
+"AND ps.cont_no      =scd.cont_no " 
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLiftOffSelect", query = 
"SELECT scd.cont_no, " 
+"  scd.cont_size, " 
+"  mct.name, " 
+"  scd.cont_status, " 
...
+"ORDER BY scd.modified_date DESC"),
    ...
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLiftOnList", query = "SELECT scd.cont_no, scd.cont_size, mct.type_in_general as name, scd.cont_status, scd.cont_gross, scd.seal_no, ps.y_slot, ps.y_row, ps.y_tier, scd.id,ps.block,scd.bl_no,ps.id FROM service_shifting scd, m_container_type mct,planning_shift_discharge ps WHERE scd.cont_type = mct.cont_type AND scd.position='04' AND scd.no_ppkb = ? AND ps.cont_no=scd.cont_no AND ps.no_ppkb=scd.no_ppkb ORDER BY scd.id DESC"),
    
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLiftOnListSelect", query = "SELECT scd.cont_no, scd.cont_size, mct.name, scd.cont_status, scd.cont_gross, scd.seal_no, ps.y_slot, ps.y_row, ps.y_tier, scd.id,ps.block,scd.bl_no,ps.id,ps.disch_port FROM service_shifting scd, m_container_type mct,planning_shift_discharge ps WHERE scd.cont_type = mct.cont_type AND scd.position='03' AND scd.no_ppkb = ? AND ps.cont_no=scd.cont_no AND ps.no_ppkb=scd.no_ppkb ORDER BY scd.id DESC"),
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findLiftableOnContainer", query = 
"SELECT pcl.cont_no, " 
+"  pcl.no_ppkb, " 
+"  mv.name vessel_name, " 
+"  pcl.block, " 
...
+"WHERE ss.position   ='03' " 
+"AND ss.cont_no      = ? " 
+"AND pcl.is_shifting = 'TRUE'"),
...
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLoadConfirmList", query = 
"SELECT p.cont_no, " 
+"  p.cont_size, " 
+"  c.type_in_general AS name, " 
+"  p.cont_status, " 
...
/*
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLoadConfirmSelectHht", query = "SELECT p.id,p.cont_no, p.no_ppkb, ps.y_slot, ps.y_row, ps.y_tier,  p.v_bay, p.v_row, p.v_tier, ps.id FROM service_shifting p, m_container_type c, m_commodity mc,planning_shift_discharge ps WHERE p.cont_type=c.cont_type  AND p.commodity_code=mc.commodity_code AND p.no_ppkb=ps.no_ppkb AND p.no_ppkb =? AND p.cont_no = ? AND p.position = ? AND p.cont_no=ps.cont_no limit 1"),
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLoadConfirmSelectHht", query = 
"SELECT p.id, " 
+"  p.cont_no, " 
+"  p.no_ppkb, " 
+"  ps.y_slot, " 
...
/*  
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLoadConfirmSelectHht2", query = "SELECT p.id,p.cont_no, p.no_ppkb, ps.y_slot, ps.y_row, ps.y_tier,  p.v_bay, p.v_row, p.v_tier, ps.id FROM service_shifting p, m_container_type c, m_commodity mc,planning_shift_discharge ps WHERE p.cont_type=c.cont_type  AND p.commodity_code=mc.commodity_code AND p.no_ppkb=ps.no_ppkb AND p.cont_no = ? AND p.position = ? AND p.cont_no=ps.cont_no limit 1")
*/
    @NamedNativeQuery(name = "ServiceShifting.Native.findByPpkbLoadConfirmSelectHht2", query = 
"SELECT p.id, " 
+"  p.cont_no, " 
+"  p.no_ppkb, " 
+"  ps.y_slot, " 
...
public class ServiceShifting implements Serializable, EntityAuditable {
    ...
    public ServiceShifting(Integer id, String contNo, short contSize, String contStatus, String overSize, String dg, String shiftTo, MasterCustomer mlo) {
        this.id = id;
        this.contNo = contNo;
        this.mlo = mlo;
