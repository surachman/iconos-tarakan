...
@NamedNativeQueries({
    @NamedNativeQuery(name = "PenumpukanSusulanService.Native.findByPpkbNReg", query = "SELECT ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
+"  ct.name, " 
...
+"JOIN m_commodity c " 
+"ON (ds.commodity_code = c.commodity_code) " 
+"JOIN m_container_type ct " 
+"ON (ds.cont_type = ct.cont_type) " 
+"WHERE ds.no_ppkb = ? " 
+"AND ds.no_reg    = ? " 
+"GROUP BY ds.job_slip, " 
+"  c.name, " 
+"  ds.cont_no, " 
+"  ds.cont_size, " 
+"  ct.name, " 
...
+"  ds.placement_date, " 
+"  ds.valid_date"),
    @NamedNativeQuery(name = "PenumpukanSusulanService.Native.generateId", query = "SELECT MAX(substr(job_slip,7,5))FROM penumpukan_susulan_service WHERE substr(job_slip,3,4) = ?"),
...
public class PenumpukanSusulanService implements Serializable, EntityAuditable {
    ...
    @SequenceGenerator(name="PENUMPUKAN_SUSULAN_SERVICE_SEQ", sequenceName="PENUMPUKAN_SUSULAN_SERVICE_SEQ", initialValue = 40, allocationSize = 1)
    @GeneratedValue(strategy=GenerationType.SEQUENCE, generator="PENUMPUKAN_SUSULAN_SERVICE_SEQ")
    private Integer id;
