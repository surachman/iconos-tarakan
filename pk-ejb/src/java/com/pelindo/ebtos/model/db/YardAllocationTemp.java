...
@NamedNativeQueries({
    @NamedNativeQuery(name = "YardAllocationTemp.Native.truncate", query = "TRUNCATE TABLE yard_allocation_temp"),
...
*/    
    @NamedNativeQuery(name = "YardAllocationTemp.Native.findIdContNotGenerate", query = 
"SELECT id_cont, " 
+"  id " 
+"FROM " 
+"  (SELECT id_cont, " 
...
+"  WHERE id_coordinat          IS NULL " 
+"  AND no_ppkb                  =? " 
+"  AND id_yard_allocation_filter=? " 
+"  ORDER BY id " 
+"  ) " 
+"WHERE rownum < 2"),
...
public class YardAllocationTemp implements Serializable, EntityAuditable {
    ...
    public YardAllocationTemp(BigDecimal id) {
        this.id = id;
    }
