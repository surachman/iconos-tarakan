...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "PluggingReeferService.Native.findPluggingReeferServiceByGatePlugging", query = "SELECT pp.job_slip,pp.no_ppkb,pp.cont_no,pp.bl_no,mm.iso_code, r.no_reg FROM plugging_reefer_service pp,m_container_type mm,registration r where pp.cont_type=mm.cont_type AND pp.no_reg=r.no_reg AND pp.job_slip=? AND pp.job_slip NOT IN (select d.job_slip from service_gate_delivery d) limit 1"),
    ...
    @NamedNativeQuery(name = "PluggingReeferService.Native.findUpdateServiceReceiving", query = "select distinct(s.id),pr.job_slip from service_receiving s,equipment e,plugging_reefer_service pr where e.cont_no=pr.cont_no AND e.no_ppkb_plug=pr.no_ppkb AND s.cont_no=pr.cont_no AND e.cont_no=s.cont_no AND s.cont_no=? AND pr.no_ppkb=? order by s.id desc LIMIT 1"),
    ...
    @NamedNativeQuery(name = "PluggingReeferService.Native.findSelectPluggingReeferServiceReceivingHht", query = "select rs.job_slip,rs.cont_no,rs.no_ppkb,null as block from plugging_reefer_service rs where rs.status='01' AND rs.job_slip IN (select sg.job_slip from service_gate_receiving sg) AND rs.cont_no=null order by rs.job_slip DESC limit 1"),
    @NamedNativeQuery(name = "PluggingReeferService.Native.findPluggingReeferServiceByDeliveryConfirmHht", query = "select pr.job_slip,pr.cont_no,pr.no_ppkb,sr.y_slot,sr.y_row,sr.y_tier,sr.block,null as is_behandle,null as is_cfs,null as is_inspection,sr.id from plugging_reefer_service pr,service_receiving sr where pr.status='02' AND pr.job_slip IN (select p.job_slip from plugging_only_rekap p) AND pr.job_slip IN (select sg.job_slip from service_gate_delivery sg)AND sr.cont_no=pr.cont_no AND pr.cont_no=? order by pr.job_slip desc limit 1"),
