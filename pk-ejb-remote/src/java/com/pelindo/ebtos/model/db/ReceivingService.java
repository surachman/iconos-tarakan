...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "ReceivingService.Native.findReceivingServiceByClosingTimeByJobSlip", query = "SELECT pp.job_slip,pp.no_ppkb,pp.cont_no,pp.seal_no,mm.iso_code, r.no_reg "
                                                                                                        ...
                                                                                                        + "WHERE pp.job_slip = ? AND ((p.close_rec >= now() AND NOT pp.dg) OR (p.close_dg >= now() AND pp.dg)) AND pp.status_cancel_loading = FALSE AND sgr.job_slip IS NULL AND pp.counter_print > 0 "
...
//    + "WHERE p.no_ppkb = ?"),
    ...
    @NamedNativeQuery(name = "ReceivingService.Native.findGateInPassableJobSlips", query = "SELECT d.job_slip "
                                                                                            + "FROM receiving_service d "
                                                                                                    + "LEFT JOIN service_gate_receiving sgr ON (d.job_slip=sgr.job_slip) "
                                                                                            + "WHERE d.job_slip LIKE ('%'|| ? ||'%') AND sgr.job_slip IS NULL AND d.counter_print > 0 "
                                                                                            + "ORDER BY d.created_date DESC"),
    @NamedNativeQuery(name = "ReceivingService.Native.findReceivingServiceByUpdateDate", query = "SELECT scd.job_slip FROM receiving_service AS scd WHERE scd.no_ppkb = ? AND scd.cont_no = ? "),
    ...
    private Boolean statusCancelJobslip = false;
    @Basic(optional = false)
    ...
    public ReceivingService(String jobSlip) {
    ...
    public Boolean getStatusCancelJobslip() {
        return statusCancelJobslip;
    }

    public void setStatusCancelJobslip(Boolean statusCancelJobslip) {
