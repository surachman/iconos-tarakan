...
@NamedNativeQueries({
    ...
    @NamedNativeQuery(name = "ReceivingBarangService.Native.findReceivingBarangServiceByAutoComplete", query = "SELECT m.jobslip FROM receiving_barang_service m WHERE m.jobslip LIKE ('%'|| ? ||'%') AND m.jobslip NOT IN (select jobslip from service_cfs_stuffing) ORDER BY substring(m.jobslip,7,5) DESC"),
    @NamedNativeQuery(name = "ReceivingBarangService.Native.findReceivingBarangServiceByGenerate", query = "SELECT MAX(substring(jobslip,7,5))FROM receiving_barang_service WHERE substring(jobslip,3,4)= ?"),
    ...
    @NamedNativeQuery(name = "ReceivingBarangService.Native.findReceivingBarangServiceByList", query = "SELECT ds.jobslip, ds.no_ppkb, ds.cont_no, ds.bl_no, c.name, ds.unit, ds.weight, ds.volume, ds.valid_date FROM receiving_barang_service ds, m_commodity c WHERE ds.commodity_code = c.commodity_code AND ds.bl_no=? AND ds.jobslip NOT IN (select jobslip from service_cfs_stuffing) ORDER BY substring(ds.jobslip,7,5) DESC"),
    @NamedNativeQuery(name = "ReceivingBarangService.Native.findReceivingBarangServiceByListAll", query = "SELECT ds.jobslip, ds.no_ppkb, ds.cont_no, ds.bl_no, c.name, ds.unit, ds.weight, ds.volume, ds.valid_date FROM receiving_barang_service ds, m_commodity c WHERE ds.commodity_code = c.commodity_code AND ds.jobslip NOT IN (select jobslip from service_cfs_stuffing) ORDER BY substring(ds.jobslip,7,5) DESC"),
